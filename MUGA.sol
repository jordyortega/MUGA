...
contract MUGA is ERC20, Ownable {
    ...
    uint256 public payoutTaxAtAmount = 5000 * (10**18);
    uint256 public maxBuy = 2000000 * (10**18);
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        require(tradingEnabled || isWhitelisted[from] || isWhitelisted[to], "MUGA: Trading is not enabled and neither address is whitelisted");
        ...
        if(from == uniswapV2Pair && !isTaxesExempt[to]) { // ONLY MAX WALLET FOR BUYING, SELLING ALWAYS WORKS
            ...
            require(token_receiver.add(amount) <= maxBuy, "MUGA: Exceeds maximum wallet amount");
        }
    ...
    function updateMaxWallet(uint256 newMax) external onlyOwner {
        require(newMax >= 2000000, "MUGA: max wallet must be 2% or higher");
